...
public class VerkaufService extends BaseService {
    ...
    public VerkaufService(Connection connection, Scanner scanner, RolleManager rolleManager, int aktuelleRolleId) {
        super(connection, scanner);
        this.sucheService = new SucheService(connection, scanner, rolleManager, aktuelleRolleId);
    ...
    public void start() {
        ...
        while (!zurueck) {
            ...
            case "1":
                ...
                break;
            case "2":
                if (rolleManager.hatRecht(aktuelleRolleId, "VERKAUF")) {
                    starteVerkaufsprozess();
                } else {
                    System.out.println("Zugriff verweigert: Kein Verkaufsrecht.");
                }
                break;
            case "3":
                zurueck = true;
        ...
        try {
            for (Map.Entry<Integer, Integer> entry : warenkorb.entrySet()) {
                ...
                if (artikel != null) {
                    ...
                    System.out.printf("%-10d | %-30s | %5d | %12s | %12s%n",
            ...
            System.out.println("=".repeat(85));
            System.out.printf("%-10s %-30s %-5s %-12s %12s%n", "", "", "", "Gesamtsumme:", String.format("%,.2f €", gesamtsumme));
    ...
    private void zeigeKategorie(String kategorie) {
        try {
            ...
            for (Artikel artikel : alleArtikel) {
                Kategorie kat = artikel.getKategorie();
                if (kat != null && kat.getBezeichnung() != null && kat.getBezeichnung().equalsIgnoreCase(kategorie)) {
            ...
            while (bleibenInKategorie) {
                ...
                for (Artikel artikel : kategorieArtikel) {
                    ...
                    System.out.printf("%-10d | %-30s | %12s | %-40s%n",
                            artikel.getArtikelID(),
                            name,
                            String.format("%,.2f €", artikel.getEinzelpreis()),
                            kommentar
